if(NOT DEFINED GUI_PATH OR NOT DEFINED OUTPUT_PATH OR NOT DEFINED SCREENSHOT_PATH)
    message(FATAL_ERROR "Missing required GUI processing pansharpen regression arguments.")
endif()
...
set(INPUT_PATH "${OUTPUT_DIR}/pansharpen_ms_input.tif")
set(PAN_PATH "${OUTPUT_DIR}/pansharpen_pan_input.bmp")
gis_gui_generate_test_tiff("${INPUT_PATH}")
gis_gui_generate_test_bmp("${PAN_PATH}")

execute_process(
    COMMAND "${GUI_PATH}"
        -platform offscreen
        --select-plugin processing
        --select-action pansharpen
        --set-param "input=${INPUT_PATH}"
        --set-param "output=${OUTPUT_PATH}"
        --set-param "pan_file=${PAN_PATH}"
        --set-param "pan_method=simple_mean"
        --auto-execute
        --quit-on-finish
...
gis_gui_assert_regression_result(
    "GUI processing pansharpen regression"
    "${GUI_EXIT_CODE}"
    "${GUI_STDOUT}"
    "${GUI_STDERR}"
    "${OUTPUT_PATH}"
