if(NOT DEFINED GUI_PATH OR NOT DEFINED GUI_TEST_DATA_HELPER_PATH OR NOT DEFINED OUTPUT_PATH OR NOT DEFINED SCREENSHOT_PATH)
    message(FATAL_ERROR "Missing required GUI terrain multi viewshed regression arguments.")
endif()

include("${CMAKE_CURRENT_LIST_DIR}/gui_regression_helpers.cmake")
get_filename_component(OUTPUT_DIR "${OUTPUT_PATH}" DIRECTORY)
set(STATUS_PATH "${OUTPUT_DIR}/status.json")
set(INPUT_PATH "${OUTPUT_DIR}/terrain_viewshed_multi_input.tif")
...
if(NOT "${PREP_EXIT_CODE}" STREQUAL "0" OR NOT EXISTS "${INPUT_PATH}")
    message(FATAL_ERROR
        "Failed to prepare terrain multi viewshed input\n"
        "exit: ${PREP_EXIT_CODE}\nstdout:\n${PREP_STDOUT}\nstderr:\n${PREP_STDERR}")
endif()

set(OBSERVER_POINTS "observer_points=116.006,39.994\;116.020,39.980")

execute_process(
    COMMAND "${GUI_PATH}"
        -platform offscreen
        --select-plugin terrain
        --select-action viewshed_multi
        --set-param "input=${INPUT_PATH}"
        --set-param "output=${OUTPUT_PATH}"
        --set-param "band=1"
        --set-param "${OBSERVER_POINTS}"
        --set-param "observer_height=2"
        --set-param "target_height=0"
        --set-param "max_distance=0"
        --auto-execute
        --quit-on-finish
...
gis_gui_assert_regression_result(
    "GUI terrain multi viewshed regression"
    "${GUI_EXIT_CODE}"
    "${GUI_STDOUT}"
    "${GUI_STDERR}"
    "${OUTPUT_PATH}"
