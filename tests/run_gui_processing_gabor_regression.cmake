if(NOT DEFINED GUI_PATH OR NOT DEFINED OUTPUT_PATH OR NOT DEFINED SCREENSHOT_PATH)
    message(FATAL_ERROR "Missing required GUI processing gabor regression arguments.")
endif()
...
set(INPUT_PATH "${OUTPUT_DIR}/gabor_input.tif")
...
execute_process(
    COMMAND "${GUI_PATH}"
        ...
        --select-action gabor_filter
        --set-param "input=${INPUT_PATH}"
        --set-param "output=${OUTPUT_PATH}"
        --set-param "band=1"
        --set-param "kernel_size=9"
        --set-param "sigma=2"
        --set-param "gabor_theta=0"
        --set-param "gabor_lambda=6"
        --set-param "gabor_gamma=0.5"
        --auto-execute
        --quit-on-finish
...
gis_gui_assert_regression_result(
    "GUI processing gabor regression"
    "${GUI_EXIT_CODE}"
    "${GUI_STDOUT}"
    "${GUI_STDERR}"
    "${OUTPUT_PATH}"
