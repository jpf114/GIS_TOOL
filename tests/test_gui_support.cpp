...
struct ActionValidationTestParam {
    std::string testName;
    std::string plugin;
    std::string action;
    ...
    friend std::ostream& operator<<(std::ostream& os, const ActionValidationTestParam& p) {
...
class ActionValidationParamTest : public ::testing::TestWithParam<ActionValidationTestParam> {};

TEST_P(ActionValidationParamTest, ValidateActionSpecificParams) {
    const auto& p = GetParam();
    const auto issue = gis::gui::validateActionSpecificParams(p.plugin, p.action, p.params);
    ASSERT_TRUE(issue.has_value()) << "plugin=" << p.plugin << " action=" << p.action;
    EXPECT_EQ(issue->key, p.expectedKey) << "plugin=" << p.plugin << " action=" << p.action;
    if (!p.expectedMessage.empty()) {
        EXPECT_EQ(issue->message, p.expectedMessage) << "plugin=" << p.plugin << " action=" << p.action;
    }
}

INSTANTIATE_TEST_SUITE_P(
    ActionValidation,
    ActionValidationParamTest,
    ::testing::Values(
        ActionValidationTestParam{
            "ClipRequiresCutlineOrExtent", "cutting", "clip",
            {{"cutline", std::string{}}, {"extent", std::array<double, 4>{0.0, 0.0, 0.0, 0.0}}},
            "extent", "参数“裁切范围”或“裁切矢量”至少填写一个"
        },
        ActionValidationTestParam{
            "VectorConvertMismatchedExtension", "vector", "convert",
            {{"output", std::string("D:/data/result.shp")}, {"format", std::string("GeoJSON")}},
            "output", "参数“输出文件”应与“输出格式”一致：GeoJSON 应使用 .geojson 或 .json"
        },
        ActionValidationTestParam{
            "FeatureStatsBandsCountMismatch", "classification", "feature_stats",
            {{"rasters", std::string("D:/a.tif,D:/b.tif")}, {"bands", std::string("1")}},
            "bands", "参数“波段列表”数量必须与“分类栅格列表”一致"
        },
        ActionValidationTestParam{
            "SvmClassifyTrainingCsvExt", "classification", "svm_classify",
            {{"input", std::string("D:/data/scene.tif")}, {"training_csv", std::string("D:/data/samples.json")}, {"output", std::string("D:/data/result.tif")}},
            "training_csv", ""
        },
        ActionValidationTestParam{
            "SvmClassifyOutputExt", "classification", "svm_classify",
            {{"input", std::string("D:/data/scene.tif")}, {"training_csv", std::string("D:/data/samples.csv")}, {"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "SvmClassifyBandsInvalid", "classification", "svm_classify",
            {{"input", std::string("D:/data/scene.tif")}, {"training_csv", std::string("D:/data/samples.csv")}, {"output", std::string("D:/data/result.tif")}, {"bands", std::string("1,0,3")}},
            "bands", ""
        },
        ActionValidationTestParam{
            "AccuracyAssessmentMissingClassified", "classification", "accuracy_assessment",
            {},
            "classified_raster", ""
        },
        ActionValidationTestParam{
            "AccuracyAssessmentMissingReference", "classification", "accuracy_assessment",
            {{"classified_raster", std::string("D:/data/classified.tif")}},
            "reference_raster", ""
        },
        ActionValidationTestParam{
            "DosCorrectionBandZero", "georef", "dos_correction",
            {{"band", 0}},
            "band", ""
        },
        ActionValidationTestParam{
            "DosCorrectionOutputExt", "georef", "dos_correction",
            {{"band", 1}, {"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "RadiometricCalibrationBandZero", "georef", "radiometric_calibration",
            {{"band", 0}},
            "band", ""
        },
        ActionValidationTestParam{
            "RadiometricCalibrationOutputExt", "georef", "radiometric_calibration",
            {{"band", 1}, {"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "RadiometricCalibrationMetadataExt", "georef", "radiometric_calibration",
            {{"output", std::string("D:/data/result.tif")}, {"metadata_file", std::string("D:/data/meta.csv")}},
            "metadata_file", ""
        },
        ActionValidationTestParam{
            "GcpRegisterGcpFileExt", "georef", "gcp_register",
            {{"gcp_file", std::string("D:/data/points.json")}},
            "gcp_file", ""
        },
        ActionValidationTestParam{
            "GcpRegisterOutputExt", "georef", "gcp_register",
            {{"gcp_file", std::string("D:/data/points.csv")}, {"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "CosineCorrectionBandZero", "georef", "cosine_correction",
            {{"band", 0}},
            "band", ""
        },
        ActionValidationTestParam{
            "CosineCorrectionSunZenithRange", "georef", "cosine_correction",
            {{"band", 1}, {"sun_zenith_deg", 95.0}},
            "sun_zenith_deg", ""
        },
        ActionValidationTestParam{
            "CosineCorrectionSunAzimuthRange", "georef", "cosine_correction",
            {{"sun_zenith_deg", 30.0}, {"sun_azimuth_deg", 400.0}},
            "sun_azimuth_deg", ""
        },
        ActionValidationTestParam{
            "CosineCorrectionOutputExt", "georef", "cosine_correction",
            {{"sun_azimuth_deg", 180.0}, {"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "MinnaertCorrectionBandZero", "georef", "minnaert_correction",
            {{"band", 0}},
            "band", ""
        },
        ActionValidationTestParam{
            "MinnaertCorrectionKZero", "georef", "minnaert_correction",
            {{"band", 1}, {"minnaert_k", 0.0}},
            "minnaert_k", ""
        },
        ActionValidationTestParam{
            "MinnaertCorrectionSunZenithRange", "georef", "minnaert_correction",
            {{"minnaert_k", 0.5}, {"sun_zenith_deg", 95.0}},
            "sun_zenith_deg", ""
        },
        ActionValidationTestParam{
            "MinnaertCorrectionSunAzimuthRange", "georef", "minnaert_correction",
            {{"sun_zenith_deg", 30.0}, {"sun_azimuth_deg", 400.0}},
            "sun_azimuth_deg", ""
        },
        ActionValidationTestParam{
            "CCorrectionBandZero", "georef", "c_correction",
            {{"band", 0}},
            "band", ""
        },
        ActionValidationTestParam{
            "CCorrectionCValueNegative", "georef", "c_correction",
            {{"band", 1}, {"c_value", -0.1}},
            "c_value", ""
        },
        ActionValidationTestParam{
            "CCorrectionSunZenithRange", "georef", "c_correction",
            {{"c_value", 0.1}, {"sun_zenith_deg", 95.0}},
            "sun_zenith_deg", ""
        },
        ActionValidationTestParam{
            "PercentileStretchDarkPercentile", "georef", "percentile_stretch",
            {{"dark_percentile", 100.0}},
            "dark_percentile", ""
        },
        ActionValidationTestParam{
            "PercentileStretchBrightLessThanDark", "georef", "percentile_stretch",
            {{"dark_percentile", 10.0}, {"bright_percentile", 10.0}},
            "bright_percentile", ""
        },
        ActionValidationTestParam{
            "PercentileStretchBrightOver100", "georef", "percentile_stretch",
            {{"bright_percentile", 101.0}},
            "bright_percentile", ""
        },
        ActionValidationTestParam{
            "RpcOrthorectifyOutputExt", "georef", "rpc_orthorectify",
            {{"output", std::string("D:/data/result.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "FilterEvenKernelSize", "processing", "filter",
            {{"kernel_size", 4}},
            "kernel_size", "参数“核大小”必须是不小于 3 的奇数"
        },
        ActionValidationTestParam{
            "GaborFilterEvenKernelSize", "processing", "gabor_filter",
            {{"kernel_size", 4}},
            "kernel_size", ""
        },
        ActionValidationTestParam{
            "GaborFilterSigmaZero", "processing", "gabor_filter",
            {{"kernel_size", 5}, {"sigma", 0.0}},
            "sigma", ""
        },
        ActionValidationTestParam{
            "GaborFilterLambdaZero", "processing", "gabor_filter",
            {{"gabor_lambda", 0.0}},
            "gabor_lambda", ""
        },
        ActionValidationTestParam{
            "GaborFilterGammaZero", "processing", "gabor_filter",
            {{"gabor_gamma", 0.0}},
            "gabor_gamma", ""
        },
        ActionValidationTestParam{
            "GlcmTextureEvenKernelSize", "processing", "glcm_texture",
            {{"kernel_size", 4}},
            "kernel_size", ""
        },
        ActionValidationTestParam{
            "GlcmTextureLevelsOne", "processing", "glcm_texture",
            {{"kernel_size", 5}, {"glcm_levels", 1}},
            "glcm_levels", ""
        },
        ActionValidationTestParam{
            "MeanShiftSpatialRadiusZero", "processing", "mean_shift_filter",
            {{"spatial_radius", 0.0}},
            "spatial_radius", ""
        },
        ActionValidationTestParam{
            "MeanShiftColorRadiusZero", "processing", "mean_shift_filter",
            {{"color_radius", 0.0}},
            "color_radius", ""
        },
        ActionValidationTestParam{
            "MeanShiftPyramidLevelNegative", "processing", "mean_shift_filter",
            {{"pyramid_level", -1}},
            "pyramid_level", ""
        },
        ActionValidationTestParam{
            "CogOutputExtension", "raster_manage", "cog",
            {{"output", std::string("D:/data/output.png")}},
            "output", ""
        },
        ActionValidationTestParam{
            "VectorSimplifyToleranceZero", "vector", "simplify",
            {{"tolerance", 0.0}},
            "tolerance", ""
        },
        ActionValidationTestParam{
            "VectorSliverRemoveOutputExt", "vector", "sliver_remove",
            {{"output", std::string("D:/data/output.csv")}},
            "output", ""
        },
        ActionValidationTestParam{
            "VectorSliverRemoveMinAreaZero", "vector", "sliver_remove",
            {{"output", std::string("D:/data/output.gpkg")}, {"min_area", 0.0}},
            "min_area", ""
        },
        ActionValidationTestParam{
            "VectorSpatialJoinOutputExt", "vector", "spatial_join",
            {{"output", std::string("D:/data/output.csv")}},
            "output", ""
        },
        ActionValidationTestParam{
            "TerrainHillshadeBandZero", "terrain", "hillshade",
            {{"band", 0}, {"z_factor", 0.0}, {"azimuth", 400.0}, {"altitude", 100.0}},
            "band", ""
        },
        ActionValidationTestParam{
            "TerrainHillshadeZFactorZero", "terrain", "hillshade",
            {{"band", 1}, {"z_factor", 0.0}, {"azimuth", 400.0}, {"altitude", 100.0}},
            "z_factor", ""
        },
        ActionValidationTestParam{
            "TerrainHillshadeAzimuthRange", "terrain", "hillshade",
            {{"band", 1}, {"z_factor", 1.0}, {"azimuth", 400.0}, {"altitude", 100.0}},
            "azimuth", ""
        },
        ActionValidationTestParam{
            "TerrainHillshadeAltitudeRange", "terrain", "hillshade",
            {{"band", 1}, {"z_factor", 1.0}, {"azimuth", 315.0}, {"altitude", 100.0}},
            "altitude", ""
        },
        ActionValidationTestParam{
            "TerrainStreamExtractThresholdZero", "terrain", "stream_extract",
            {{"band", 1}, {"z_factor", 1.0}, {"accum_threshold", 0.0}},
            "accum_threshold", ""
        },
        ActionValidationTestParam{
            "TerrainProfileExtractPathEmpty", "terrain", "profile_extract",
            {{"band", 1}, {"profile_path", std::string("")}},
            "profile_path", ""
        },
        ActionValidationTestParam{
            "TerrainProfileExtractOutputExt", "terrain", "profile_extract",
            {{"profile_path", std::string("116.0,40.0;116.1,39.9")}, {"output", std::string("D:/data/profile.json")}},
            "output", ""
        },
        ActionValidationTestParam{
            "TerrainViewshedObserverHeightNegative", "terrain", "viewshed",
            {{"observer_height", -1.0}},
            "observer_height", ""
        },
        ActionValidationTestParam{
            "TerrainViewshedMultiPointsEmpty", "terrain", "viewshed_multi",
            {{"observer_points", std::string("")}},
            "observer_points", ""
        },
        ActionValidationTestParam{
            "TerrainViewshedMultiTargetHeightNegative", "terrain", "viewshed_multi",
            {{"observer_points", std::string("116.0,40.0;116.1,39.9")}, {"target_height", -1.0}},
            "target_height", ""
        },
        ActionValidationTestParam{
            "TerrainViewshedMultiMaxDistanceNegative", "terrain", "viewshed_multi",
            {{"observer_points", std::string("116.0,40.0;116.1,39.9")}, {"max_distance", -1.0}},
            "max_distance", ""
        }
    )
);
...
TEST(GuiSupportTest, FormatDurationTextUsesMillisecondsForShortTasks) {
    EXPECT_EQ(gis::gui::formatDurationText(250), QStringLiteral("250 ms"));
}

TEST(GuiSupportTest, FormatDurationTextUsesSecondsForLongerTasks) {
    EXPECT_EQ(gis::gui::formatDurationText(2500), QStringLiteral("2.5 s"));
}
