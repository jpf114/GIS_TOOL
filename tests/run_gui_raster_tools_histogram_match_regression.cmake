if(NOT DEFINED GUI_PATH OR NOT DEFINED OUTPUT_PATH OR NOT DEFINED SCREENSHOT_PATH)
    message(FATAL_ERROR "Missing required GUI raster-tools histogram-match regression arguments.")
endif()
...
set(INPUT_PATH "${OUTPUT_DIR}/raster_tools_histogram_match_input.tif")
set(REFERENCE_PATH "${OUTPUT_DIR}/raster_tools_histogram_match_reference.tif")
gis_gui_generate_test_tiff("${INPUT_PATH}")
gis_gui_generate_test_tiff("${REFERENCE_PATH}")

execute_process(
    COMMAND "${GUI_PATH}"
        -platform offscreen
        --select-plugin raster_tools
        --select-action histogram_match
...
gis_gui_assert_regression_result(
    "GUI raster-tools histogram-match regression"
    "${GUI_EXIT_CODE}"
    "${GUI_STDOUT}"
    "${GUI_STDERR}"
    "${OUTPUT_PATH}"
