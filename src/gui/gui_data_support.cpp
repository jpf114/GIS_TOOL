...
QString genericActionDisplayName(const std::string& actionKey) {
    static const std::map<std::string, QString> kLabels = {
        {"accuracy_assessment", QStringLiteral("精度评估")},
        {"adjacency", QStringLiteral("邻接关系分析")},
        {"arvi", QStringLiteral("ARVI（大气阻抗植被指数）")},
        {"assign_srs", QStringLiteral("赋予坐标系")},
        {"aspect", QStringLiteral("坡向")},
        {"awei", QStringLiteral("AWEI（自动水体提取指数）")},
        {"band_math", QStringLiteral("波段运算")},
        {"bi", QStringLiteral("BI（亮度指数）")},
        {"boundary", QStringLiteral("边界提取")},
        {"bsi", QStringLiteral("BSI（裸土指数）")},
        {"gabor_filter", QStringLiteral("Gabor 滤波")},
        {"c_correction", QStringLiteral("C 校正")},
        {"centroid", QStringLiteral("质心提取")},
        {"change", QStringLiteral("变化检测")},
        {"cog", QStringLiteral("COG 生成")},
        {"colormap", QStringLiteral("伪彩色渲染")},
        {"connected_components", QStringLiteral("连通组件分析")},
        {"contour", QStringLiteral("轮廓提取")},
        {"convert", QStringLiteral("格式转换")},
        {"convex_hull", QStringLiteral("凸包提取")},
        {"cosine_correction", QStringLiteral("余弦校正")},
        {"corner", QStringLiteral("角点检测")},
        {"curvature", QStringLiteral("曲率")},
        {"cut_fill", QStringLiteral("挖填方分析")},
        {"custom_index", QStringLiteral("自定义指数")},
        {"dangling_endpoint_check", QStringLiteral("悬挂端点检查")},
        {"detect", QStringLiteral("特征检测")},
        {"difference", QStringLiteral("差集分析")},
        {"dissolve", QStringLiteral("融合")},
        {"dos_correction", QStringLiteral("DOS 大气校正")},
        {"duplicate_point_check", QStringLiteral("重复点检查")},
        {"ecc_register", QStringLiteral("ECC 配准")},
        {"endpoints_extract", QStringLiteral("端点提取")},
        {"envelope", QStringLiteral("外接矩形")},
        {"evi", QStringLiteral("EVI（增强植被指数）")},
        {"evi2", QStringLiteral("EVI2（二波段增强植被指数）")},
        {"feature_stats", QStringLiteral("地物分类统计")},
        {"fill_sinks", QStringLiteral("填洼")},
        {"flow_accumulation", QStringLiteral("汇流累积")},
        {"flow_direction", QStringLiteral("流向")},
        {"reproject", QStringLiteral("重投影")},
        {"gcp_register", QStringLiteral("控制点配准")},
        {"geom_metrics", QStringLiteral("几何属性统计")},
        {"glcm_texture", QStringLiteral("GLCM 纹理")},
        {"gndvi", QStringLiteral("GNDVI（绿光归一化植被指数）")},
        {"hillshade", QStringLiteral("山体阴影")},
        {"histogram_match", QStringLiteral("直方图匹配")},
        {"hole_check", QStringLiteral("孔洞检查")},
        {"hough", QStringLiteral("霍夫变换")},
        {"interior_point", QStringLiteral("内部点提取")},
        {"intersect", QStringLiteral("交集分析")},
        {"kmeans", QStringLiteral("KMeans 聚类")},
        {"buffer", QStringLiteral("缓冲区")},
        {"match", QStringLiteral("特征匹配")},
        {"max_likelihood_classify", QStringLiteral("最大似然分类")},
        {"merge_bands", QStringLiteral("波段合并")},
        {"mean_shift_filter", QStringLiteral("Mean Shift 滤波")},
        {"midpoints_extract", QStringLiteral("中点提取")},
        {"minnaert_correction", QStringLiteral("Minnaert 校正")},
        {"mndwi", QStringLiteral("MNDWI（改进型归一化水体指数）")},
        {"mosaic", QStringLiteral("影像镶嵌")},
        {"multipart_check", QStringLiteral("多部件检查")},
        {"nbr", QStringLiteral("NBR（归一化烧毁指数）")},
        {"ndbi", QStringLiteral("NDBI（归一化建筑指数）")},
        {"ndmi", QStringLiteral("NDMI（归一化水分指数）")},
        {"ndvi", QStringLiteral("NDVI（归一化植被指数）")},
        {"ndwi", QStringLiteral("NDWI（归一化水体指数）")},
        {"nearest", QStringLiteral("最近邻分析")},
        {"nodata", QStringLiteral("NoData 设置")},
        {"info", QStringLiteral("信息查看")},
        {"osavi", QStringLiteral("OSAVI（优化土壤调节植被指数）")},
        {"overlap_check", QStringLiteral("重叠检查")},
        {"overviews", QStringLiteral("金字塔构建")},
        {"pansharpen", QStringLiteral("全色锐化")},
        {"percentile_stretch", QStringLiteral("百分位拉伸")},
        {"plan_curvature", QStringLiteral("平面曲率")},
        {"polygonize", QStringLiteral("栅格转面")},
        {"profile_curvature", QStringLiteral("剖面曲率")},
        {"profile_extract", QStringLiteral("剖面提取")},
        {"proximity", QStringLiteral("欧氏距离")},
        {"random_forest_classify", QStringLiteral("随机森林分类")},
        {"radiometric_calibration", QStringLiteral("辐射定标")},
        {"raster_overlay", QStringLiteral("栅格叠加")},
        {"rasterize", QStringLiteral("矢量转栅格")},
        {"reclassify", QStringLiteral("重分类")},
        {"register", QStringLiteral("影像配准")},
        {"repair", QStringLiteral("几何修复")},
        {"reservoir_volume", QStringLiteral("库容计算")},
        {"roughness", QStringLiteral("粗糙度")},
        {"rpc_orthorectify", QStringLiteral("RPC 正射校正")},
        {"savi", QStringLiteral("SAVI（土壤调节植被指数）")},
        {"simplify", QStringLiteral("几何简化")},
        {"singlepart", QStringLiteral("多部件转单部件")},
        {"sliver_remove", QStringLiteral("碎片面清理")},
        {"slope", QStringLiteral("坡度")},
        {"spatial_join", QStringLiteral("空间连接")},
        {"split", QStringLiteral("分块")},
        {"stream_extract", QStringLiteral("河网提取")},
        {"stitch", QStringLiteral("图像拼接")},
        {"svm_classify", QStringLiteral("SVM 分类")},
        {"clip", QStringLiteral("裁切")},
        {"tile", QStringLiteral("栅格切片")},
        {"topology_check", QStringLiteral("拓扑检查")},
        {"template_match", QStringLiteral("模板匹配")},
        {"tpi", QStringLiteral("TPI 地形位置指数")},
        {"transform", QStringLiteral("坐标转换")},
        {"tri", QStringLiteral("TRI 地形崎岖度指数")},
        {"ui", QStringLiteral("UI（城镇指数）")},
        {"union", QStringLiteral("并集分析")},
        {"vertices_extract", QStringLiteral("顶点提取")},
        {"viewshed", QStringLiteral("视域分析")},
        {"viewshed_multi", QStringLiteral("多点视域")},
        {"watershed", QStringLiteral("流域划分")},
        {"zonal_stats", QStringLiteral("分区统计")},
    };
...
std::optional<ActionValidationIssue> validateActionSpecificParams(
    ...
    const std::string& actionKey,
    const std::map<std::string, gis::framework::ParamValue>& params) {
    return gis::framework::validateActionSpecificParams(pluginName, actionKey, params);
}
...
std::vector<gis::framework::ParamSpec> buildEffectiveGuiParamSpecs(
    ...
    for (const auto& spec : specs) {
        ...
        adjustedSpec.required = requiredKeys.empty()
            ? spec.required
            : requiredKeys.count(spec.key) > 0;
