...
inline QString globalStyleSheet() {
    return QString(
        ...
        "QProgressBar {"
          "  border: none; border-radius: %33px; background: %33; height: %34px; text-align: center;"
        "  color: %9; font-weight: 600; font-size: 11px;"
        "}"
        "QProgressBar::chunk { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #5CB8FF, stop:1 #2E7EC9); border-radius: %33px; }"
