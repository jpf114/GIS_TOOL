...
- `build/debug/src/gui/Debug/gis-gui.exe -platform offscreen --self-test` 正常
- Debug 全量 `ctest --test-dir build/debug -C Debug --output-on-failure` 本地执行超过 5 分钟，未在当轮跑完；发布前仍需完整执行
- GUI 的任务队列、批量处理、耗时统计、错误提示等主链能力可用
...
- GUI 左侧导航已将 `raster_manage / raster_inspect / raster_render / raster_math` 归并为单一“栅格工具”入口
- `classification` 在 GUI 中统一以“分类统计”作为主项，具体动作作为子功能呈现
- GUI 与 CLI 最终都落到同一套插件执行链路
